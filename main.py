...
pygame.init()
timer = 1000  # параментр от которого зависит через какое время будут новые шарики
FPS = 10  # FPS...
...
while not finished:  #
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            give_score(points)
            finished = True
        elif event.type == pygame.MOUSEBUTTONDOWN:  # Проверяет расстояние от щелчка до всех шариков
            for i in range(chislo):
                if r[i] ** 2 >= (((x[i] - pygame.mouse.get_pos()[0]) ** 2) +
                                 ((y[i] - pygame.mouse.get_pos()[1]) ** 2)):
                    score = score + 10
                    score = score + chislo - 1
                    cychect[i] = False
                else:
                    score = score - 1
